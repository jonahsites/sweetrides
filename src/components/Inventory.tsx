...
const Inventory: React.FC<InventoryProps> = ({ onClose }) => {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[200] bg-luxury-black overflow-y-auto px-6 py-10 md:px-16 font-sans"
    >
      <div className="max-w-[1400px] mx-auto">
        {/* Header */}
        <div className="flex justify-between items-start mb-20">
          <div>
            <div className="text-accent text-[10px] uppercase tracking-[0.5em] font-black mb-4 font-sans">The Sweet Fleet by Sweet EV Rides</div>
            <h2 className="font-display text-5xl md:text-7xl leading-none">
              RIDE <span className="text-green-outline">SWEET</span><br/>OR RIDE HOME.
            </h2>
          </div>
          <button
            onClick={onClose}
            className="p-4 border border-accent/30 rounded-full hover:bg-accent/10 transition-colors pointer-events-auto"
          >
            <X size={24} />
          </button>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 justify-between items-center mb-16 border-b border-white/5 pb-10">
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${
                  activeCategory === cat ? 'bg-accent text-black font-black' : 'bg-white/5 border border-accent/15 text-white/40 hover:text-accent hover:border-accent/40'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="relative w-full lg:w-96">
            ...
            <input 
              ...
              className="w-full bg-white/5 border border-accent/15 rounded-xl py-4 pl-12 pr-6 text-sm focus:outline-none focus:border-accent/50 placeholder:text-white/20"
            />
          </div>
        </div>
          ...
          <AnimatePresence mode="popLayout">
            {filteredCars.map((car) => (
              <motion.div
                ...
                className="group relative bg-[#0D1A08] border border-accent/8 hover:border-accent/30 transition-all overflow-hidden rounded-2xl"
              >
                {/* Image */}
                ...
                <div className="p-8">
                  ...
                  <h3 className="font-display text-xl mb-4 group-hover:text-accent transition-colors">{car.name}</h3>
                ...
                {/* Bottom Bar */}
                <div className="absolute bottom-0 left-0 w-full h-[3px] bg-accent scale-x-0 group-hover:scale-x-100 transition-transform origin-left rounded-b-2xl" />
